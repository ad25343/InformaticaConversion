...
def _apply_cred_overrides(cred_vars: dict, org_overrides: dict) -> None:
    """Apply org-level credential env-var overrides in-place."""
    for var_key, new_env_var in org_overrides.items():
        if var_key in cred_vars:
            cred_vars[var_key] = new_env_var


def _cred_var_line(var_key: str, env_var: str, default: str) -> str:
    """Return one profiles.yml credential line."""
    if default:
        return f"      {var_key}: \"{{{{{{ env_var('{env_var}', '{default}') }}}}}}\""
    return f"      {var_key}: \"{{{{{{ env_var('{env_var}') }}}}}}\""


def _prod_env_var(env_var: str) -> str:
    """Return the PROD variant of an env-var name."""
    return env_var if env_var.endswith("_PROD") else env_var + "_PROD"


def _build_profiles_lines(
    mapping_name: str,
    adapter: str,
    cred_vars: dict,
    defaults: dict,
) -> list[str]:
    """Build the profiles.yml line list for both dev and prod targets."""
    ...
    for var_key, env_var in cred_vars.items():
        lines.append(_cred_var_line(var_key, env_var, defaults.get(var_key, "")))
    lines.append(f"      threads: {threads}")
    lines.append("    prod:")
    lines.append(f"      type: {adapter}")
    for var_key, env_var in cred_vars.items():
        prod_var = _prod_env_var(env_var)
        prod_default = defaults.get(var_key + "_prod", defaults.get(var_key, ""))
        lines.append(_cred_var_line(var_key, prod_var, prod_default))
    lines.append(f"      threads: {defaults.get('threads_prod', threads * 2)}")
    return lines
...
def _get_profiles_yml_from_registry(mapping_name: str, db_type: str) -> str | None:
    ...
    try:
        ...
        _apply_cred_overrides(cred_vars, get_warehouse_cred_overrides().get(key, {}))
        lines = _build_profiles_lines(
            mapping_name,
...
def _match_warehouse_keyword(ct: str) -> Optional[str]:
    """Return warehouse name if ct matches any keyword, else None."""
    for keyword, name in _WAREHOUSE_KEYWORDS:
        if keyword in ct:
            return name
    return None


def _session_connections(spr: Optional[SessionParseReport]) -> list:
    """Return the connection list from spr's session_config, or empty list."""
    sc = spr.session_config if spr else None
    return sc.connections if sc else []


def _detect_warehouse(session_parse_report: Optional[SessionParseReport]) -> str:
    """Determine warehouse type from session connection metadata."""
    for conn in _session_connections(session_parse_report):
        ct = (conn.connection_type or "").upper()
        matched = _match_warehouse_keyword(ct)
        if matched:
            return matched
    return "postgres"


def _build_profiles_yml(mapping_slug: str, warehouse: str) -> str:
    """Return profiles.yml content from registry or template."""
    ...
    return template.replace("{mapping_name}", mapping_slug)
...
def build_dbt_runtime_artifacts(
    ...
    warehouse    = _detect_warehouse(session_parse_report)
    adapter_pkg  = _DBT_ADAPTER_PACKAGES.get(warehouse, "dbt-postgres>=1.7,<2.0")

    return {
        "profiles.yml": _build_profiles_yml(mapping_slug, warehouse),
        "requirements.txt": "\n".join([
