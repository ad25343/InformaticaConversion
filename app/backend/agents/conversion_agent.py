...
def _load_prompt_template(stack_name: str, default: str) -> str:
    ...
    try:
        ...
        import re as _re
        prompt_path = Path(__file__).parent.parent.parent / "prompts" / f"{stack_name}_system.j2"
...
# G2: _DW_AUDIT_RULES is now loaded from org_config.yaml at runtime.
# See org_config_loader.build_dw_audit_rules() for defaults.
...
# G3: Allow system prompts to be overridden by Jinja2 template files.
PYSPARK_SYSTEM = _load_prompt_template("pyspark", PYSPARK_SYSTEM)
DBT_SYSTEM     = _load_prompt_template("dbt",     DBT_SYSTEM)
PYTHON_SYSTEM  = _load_prompt_template("python",  PYTHON_SYSTEM)
...
def _get_profiles_yml_from_registry(mapping_name: str, db_type: str) -> str | None:
    ...
    try:
        ...
        lines = [
            f"# profiles.yml — generated by Informatica Conversion Tool",
            f"# Set the environment variables below before running dbt.",
            f"{mapping_name}:",
            f"  target: dev",
            f"  outputs:",
            f"    dev:",
...
async def convert(
    ...
    # When the classifier identified a matching pattern with confidence ≥ LOW,
    # emit a starter YAML config that the developer can fill in and use to run
    # the mapping via the etl_patterns library instead of the generated code.
