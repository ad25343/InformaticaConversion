...
def _scan_graph_expressions(graph: dict) -> dict[str, list[str]]:
    """Scan graph for high-risk expression patterns. Returns {category: [examples]}."""
    detected: dict[str, list[str]] = {}
    for m in graph.get("mappings", []):
        for t in m.get("transformations", []):
            for expr_entry in t.get("expressions", []):
                expr_str = _extract_expr_str(expr_entry)
                if not expr_str:
                    continue
                _classify_expression(expr_str, detected)
    return detected


def _extract_expr_str(expr_entry: object) -> str:
    """Extract the expression string from either a dict or str entry."""
    if isinstance(expr_entry, dict):
        return expr_entry.get("expression", "")
    if isinstance(expr_entry, str):
        return expr_entry
    return ""


def _classify_expression(expr_str: str, detected: dict[str, list[str]]) -> None:
    """Classify expression into categories by pattern match, in-place."""
    for cat, pattern in _EXPR_PATTERNS:
        if pattern.search(expr_str):
            bucket = detected.setdefault(cat, [])
            if len(bucket) < 2:
                bucket.append(expr_str[:80])


def _build_test_header(mapping_name: str, ts: str, safe_name: str, is_spark: bool, categories: dict) -> list[str]:
    """Build the file docstring and import lines."""
    lines: list[str] = [
        f'"""',
        ...
        f"",
        f"Detected risk categories: {', '.join(categories.keys())}",
        f'"""',
...
def _iif_test_lines(examples: list[str]) -> list[str]:
    ex_comment = "\n".join(f"#   {e}" for e in examples)
    return [
        "# IIF — NULL-branch semantics",
        "# Informatica: NULL condition input evaluates the FALSE branch (not NULL propagation)",
        "# Detected example(s):",
        ex_comment,
        "# " + "─" * 75,
        "",
...
def _decode_test_lines(examples: list[str]) -> list[str]:
    ex_comment = "\n".join(f"#   {e}" for e in examples)
    return [
        "# " + "─" * 75,
...
def _date_test_lines(examples: list[str]) -> list[str]:
    ex_comment = "\n".join(f"#   {e}" for e in examples)
    return [
        "# " + "─" * 75,
...
def _string_test_lines(examples: list[str]) -> list[str]:
    ex_comment = "\n".join(f"#   {e}" for e in examples)
    return [
        "# " + "─" * 75,
...
def _aggregation_spark_lines(examples: list[str]) -> list[str]:
    ex_comment = "\n".join(f"#   {e}" for e in examples)
    return [
        "# " + "─" * 75,
...
def _aggregation_pandas_lines(examples: list[str]) -> list[str]:
    ex_comment = "\n".join(f"#   {e}" for e in examples)
    return [
        "# " + "─" * 75,
        "# Aggregation — empty partition must return NULL (not 0) to match Informatica",
...
_CATEGORY_LINE_BUILDERS = {
    "iif":    _iif_test_lines,
    "decode": _decode_test_lines,
    "date":   _date_test_lines,
    "string": _string_test_lines,
}


def _build_category_lines(detected_categories: dict[str, list[str]], is_spark: bool) -> list[str]:
    """Append test lines for each detected category."""
    lines: list[str] = []
    for cat, builder in _CATEGORY_LINE_BUILDERS.items():
        if cat in detected_categories:
            lines.extend(builder(detected_categories[cat]))
    if "aggregation" in detected_categories:
        examples = detected_categories["aggregation"]
        agg_lines = _aggregation_spark_lines(examples) if is_spark else _aggregation_pandas_lines(examples)
...
def generate_expression_boundary_tests(
    ...
    detected_categories = _scan_graph_expressions(graph)
    ...
    lines = _build_test_header(mapping_name, ts, safe_name, is_spark, detected_categories)
    lines.extend(_build_category_lines(detected_categories, is_spark))
    lines += [
        "# " + "─" * 75,
