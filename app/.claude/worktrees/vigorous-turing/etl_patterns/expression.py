...
def _eval_arithmetic(expr: str, row: dict) -> Any:
    ...
    try:
        return _safe_arith_eval(substituted)
    except ExpressionError:
        raise
    except Exception as exc:
        raise ExpressionError(
...
def _safe_arith_eval(expr: str) -> Any:
    """Evaluate a numeric arithmetic expression using the AST — no eval()."""
    try:
        tree = ast.parse(expr.strip(), mode="eval")
    except SyntaxError as exc:
        raise ExpressionError(f"Invalid arithmetic expression: {expr!r}") from exc
    return _eval_arith_node(tree.body)


def _eval_arith_node(node: ast.expr) -> Any:
    """Recursively evaluate an arithmetic AST node (numbers and basic operators only)."""
    if isinstance(node, ast.Constant):
        if node.value is None or isinstance(node.value, (int, float)):
            return node.value
        raise ExpressionError(f"Non-numeric constant in arithmetic: {node.value!r}")
    ...
    if isinstance(node, ast.BinOp):
        ...
        if isinstance(op, ast.Div):
            if right == 0:
                raise ExpressionError("Division by zero in arithmetic expression")
            return left / right
        if isinstance(op, ast.Mod):
            if right == 0:
                raise ExpressionError("Modulo by zero in arithmetic expression")
            return left % right
        raise ExpressionError(f"Unsupported arithmetic operator: {type(op).__name__}")
    if isinstance(node, ast.UnaryOp) and isinstance(node.op, ast.USub):
        operand = _eval_arith_node(node.operand)
        return None if operand is None else -operand
    raise ExpressionError(
        f"Unsupported node type in arithmetic expression: {type(node).__name__}"
    )
