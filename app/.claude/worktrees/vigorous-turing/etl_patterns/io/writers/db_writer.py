...
def _assert_safe_identifier(value: str, kind: str) -> None:
    """Raise ConfigError if *value* is not a safe SQL identifier (alphanumeric + underscore)."""
    if not isinstance(value, str) or not _SAFE_IDENT_RE.match(value):
        raise ConfigError(
            f"Unsafe SQL identifier for {kind}: {value!r}. "
            ...
            for i in range(0, len(df), chunksize):
                ...
                with engine.begin() as conn:
                    ...
                    for col in unique_key:
                        _assert_safe_identifier(col, "unique_key column")
